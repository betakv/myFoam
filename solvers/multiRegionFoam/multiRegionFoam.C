...
#include "compressibleSlowCourantNo.H"
...
int main(int argc, char *argv[])
{
    ...
    #include "slowinitContinuityErrs.H"
    ...
    while (runTime.run())
    {
        ...
        if (nOuterCorr != 1)
        {
            ...
            {
                #include "storeOldSlowFluidFields.H"
            }
        }
        ...
        for (int oCorr=0; oCorr<nOuterCorr; ++oCorr)
        {
            ...
            forAll(slowFluidRegions, i)
            {
                Info<< "\nSolving for slow fluid region "
                    << slowFluidRegions[i].name() << endl;
                #include "setRegionSlowFluidFields.H"
                #include "readSlowFluidMultiRegionPIMPLEControls.H"
