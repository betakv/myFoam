...
#include "profileModelUpList.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::profileModelUpList::profileModelUpList
(
    const dictionary& dict,
    const bool readFields
)
:
    PtrList<profileModelUp>(),
    dict_(dict)
{
    if (readFields)
    {
        ...
        if (modelNames.size())
        {
            ...
            forAll(modelNames, i)
            {
                ...
                (
                    i,
                    profileModelUp::New(dict.subDict(modelName))
                );
            }
        }
        else
...
void Foam::profileModelUpList::connectBlades
(
...
) const
{
    ...
    forAll(names, bI)
    {
        ...
        forAll(*this, pI)
        {
            const profileModelUp& pm = this->operator[](pI);
        ...
        if (index == -1)
        {
            ...
            forAll(*this, i)
            {
                const profileModelUp& pm = this->operator[](i);
